...
class RegexPresetManager {
     ...
     */
    captureCurrentState() {
        const globalScripts = this.regexListToPresetItems(
            getScriptsByType(scriptTypes.GLOBAL),
        );
        const scopedScripts = this.regexListToPresetItems(
            getScriptsByType(scriptTypes.SCOPED),
        );
        const presetScripts = this.regexListToPresetItems(
            getScriptsByType(scriptTypes.PRESET),
        );
    ...
    hasStateChanged(state1, state2) {
        ...
        if (
            global1.length !== global2.length ||
            scoped1.length !== scoped2.length
        ) {
            return true;
        }
     ...
     */
    setupEventListeners() {
        this.presetSelect = /** @type {HTMLSelectElement} */ (
            document.getElementById("regex_presets")
        );
        if (!this.presetSelect) {
            console.error(
                "RegexPresetManager: Could not find preset select element in the DOM.",
            );
            return;
        }
        ...
        this.presetSelect.addEventListener("change", async (event) => {
            ...
            const fromSlashCommand =
                ...
                event?.detail?.fromSlashCommand === true;
        ...
        this.presetCreateButton = document.getElementById(
            "regex_preset_create",
        );
        if (!this.presetCreateButton) {
            console.error(
                "RegexPresetManager: Could not find preset create button in the DOM.",
            );
            return;
        }
        ...
        this.presetUpdateButton = document.getElementById(
            "regex_preset_update",
        );
        if (!this.presetUpdateButton) {
            console.error(
                "RegexPresetManager: Could not find preset update button in the DOM.",
            );
            return;
        }
        ...
        if (!this.presetApplyButton) {
            console.error(
                "RegexPresetManager: Could not find preset apply button in the DOM.",
            );
            return;
        }
        ...
        this.presetDeleteButton = document.getElementById(
            "regex_preset_delete",
        );
        if (!this.presetDeleteButton) {
            console.error(
                "RegexPresetManager: Could not find preset delete button in the DOM.",
            );
            return;
        }
        ...
        this.presetDeleteButton.addEventListener("click", async () => {
            ...
            const newSelectedPresetId = extension_settings.regex_presets.find(
                (p) => p.isSelected,
            )?.id;
        ...
        const selectedPreset = extension_settings.regex_presets?.find(
            (p) => p.isSelected,
        );
        if (selectedPreset) {
            ...
            SlashCommand.fromProps({
                ...
                callback: (args, name) => {
                    ...
                    if (name) {
                        ...
                        const foundId = extension_settings.regex_presets.find(
                            (p) =>
                                equalsIgnoreCaseAndAccents(p.id, name) ||
                                equalsIgnoreCaseAndAccents(p.name, name),
                        )?.id;
                        ...
                        !quiet &&
                            toastr.warning(`Regex preset "${name}" not found`);
                        return "";
                    }
                ...
                namedArgumentList: [
                    SlashCommandNamedArgument.fromProps({
                        name: "quiet",
                        description:
                            "Suppress the toast message on preset change",
    ...
    async applyPresetList({ presetList, targetList, saveFunction }) {
        ...
        targetList.sort((a, b) => {
            ...
            return (
                aIndex - bIndex || targetList.indexOf(a) - targetList.indexOf(b)
            );
        });
    ...
    async applyPreset(presetId) {
        const preset = extension_settings.regex_presets.find(
            (p) => p.id === presetId,
        );
        if (!preset) {
    ...
    async savePreset(presetId, isUpdate) {
        ...
        const name = isUpdate
            ...
            : await Popup.show.input(
                  t`Enter a name for the new regex preset:`,
                  "",
              );
        ...
        const preset = {
            ...
            name: name,
            isSelected: false,
            global: this.regexListToPresetItems(
                getScriptsByType(scriptTypes.GLOBAL),
            ),
            scoped: this.regexListToPresetItems(
                getScriptsByType(scriptTypes.SCOPED),
            ),
            preset: this.regexListToPresetItems(
                getScriptsByType(scriptTypes.PRESET),
            ),
        };
        ...
        toastr.success(
            isUpdate ? t`Regex preset updated` : t`Regex preset saved`,
        );
    }
    ...
    async deletePreset(presetId) {
        const presetIndex = extension_settings.regex_presets.findIndex(
            (p) => p.id === presetId,
        );
        if (presetIndex === -1) {
...
function setMoveButtonsVisibility() {
    const hasGlobalScripts =
        $(
            "#saved_regex_scripts .regex-script-label:has(.regex_bulk_checkbox:checked)",
        ).length > 0;
    const hasScopedScripts =
        $(
            "#saved_scoped_scripts .regex-script-label:has(.regex_bulk_checkbox:checked)",
        ).length > 0;
    const hasPresetScripts =
        $(
            "#saved_preset_scripts .regex-script-label:has(.regex_bulk_checkbox:checked)",
        ).length > 0;
    hideOrShowElement(
        $("#bulk_regex_move_to_global"),
        hasScopedScripts || hasPresetScripts,
    );
    hideOrShowElement(
        $("#bulk_regex_move_to_scoped"),
        hasGlobalScripts || hasPresetScripts,
    );
    hideOrShowElement(
        $("#bulk_regex_move_to_preset"),
        hasGlobalScripts || hasScopedScripts,
    );
}
 ...
 * @param {number} scriptType global? scoped? preset?
...
async function saveRegexScript(
    ...
    if (scriptType === scriptTypes.PRESET) {
        ...
        if (
            !extension_settings.preset_allowed_regex[main_api].includes(
                presetName,
            )
        ) {
...
async function moveRegexScript(
    script,
    toType,
    fromType = null,
    saveSettings = true,
    ...
    while ((reMatch = backrefRegex.exec(pattern)) !== null) {
        ...
        if (backref === "$$") {
            ...
            container.appendChild(
                document.createTextNode(match.input.substring(0, match.index)),
            );
        } else if (backref === "$'") {
            container.appendChild(
...
function executeRegexScriptForDebugging(script, text) {
    ...
    const globalRegex = new RegExp(
        ...
        originalRegex.flags.includes("g")
            ? originalRegex.flags
            : originalRegex.flags + "g",
    );
    ...
    try {
        for (const match of matches) {
            ...
            while (
                (reMatch = backrefRegex.exec(script.replaceString)) !== null
            ) {
                const literalPart = script.replaceString.substring(
                    lastPatternIndex,
                    reMatch.index,
                );
                ...
                if (backref === "$$") {
                    ...
                    const part = match.input.substring(
            ...
            const finalLiteralPart =
                script.replaceString.substring(lastPatternIndex);
            ...
            highlightedOutput += buildReplacementHtml(
                match,
...
function populateDebuggerRuleList(container) {
    ...
    if (!rulesContainer.length || !ruleTemplate.length) {
        console.error(
            "Regex Debugger: Could not find rule list or template in the DOM.",
        );
        return;
    }
    ...
    if (!allScripts || allScripts.length === 0) {
        rulesContainer.append(
            '<div class="regex-debugger-no-rules">' +
                t`No regex rules found.` +
                "</div>",
        );
        return;
    }
    ...
    const globalScriptIds = new Set(
        getScriptsByType(scriptTypes.GLOBAL).map((s) => s.id),
    );
    const scopedScriptIds = new Set(
        getScriptsByType(scriptTypes.SCOPED).map((s) => s.id),
    );
    const globalScripts = [];
    const scopedScripts = [];
    const presetScripts = [];
    ...
    allScripts.forEach((script) => {
        ...
        if (globalScriptIds.has(script.id)) {
            ...
            // @ts-ignore
            scriptCopy.type = scriptTypes.PRESET;
            presetScripts.push(scriptCopy);
        }
    });

    container.data("allScripts", [
        ...globalScripts,
        ...scopedScripts,
        ...presetScripts,
    ]);
    ...
    if (globalScripts.length > 0) {
        rulesContainer.append(
            '<div class="list-header regex-debugger-list-header">' +
                t`Global Rules` +
                "</div>",
        );
        const globalList = $(
            '<ul id="regex_debugger_rules_global" class="sortable-list"></ul>',
        );
        globalScripts.forEach((script) =>
            globalList.append(renderRule(script)),
        );
    ...
    if (scopedScripts.length > 0) {
        ...
        const scopedList = $(
            '<ul id="regex_debugger_rules_scoped" class="sortable-list"></ul>',
        );
        scopedScripts.forEach((script) =>
            scopedList.append(renderRule(script)),
        );
    ...
    if (presetScripts.length > 0) {
        ...
        const presetList = $(
            '<ul id="regex_debugger_rules_preset" class="sortable-list"></ul>',
        );
        presetScripts.forEach((script) =>
            presetList.append(renderRule(script)),
        );
...
async function onRegexDebuggerOpenClick() {
    const templateContent = await renderExtensionTemplateAsync(
        "regex",
        "debugger",
    );
    ...
    debuggerHtml.find("#regex_debugger_run_test").on("click", function () {
        ...
        orderedRuleIds.forEach((scriptId) => {
            const ruleElement = $(
                `#regex_debugger_rules [data-id="${scriptId}"]`,
            );
            ...
            if (script) {
                const result = executeRegexScriptForDebugging(
                    script,
                    textForNextStep,
                );
                ...
                stepElement
                    .find(">:first-child")
                    .attr("id", `step-result-${script.id}`);
                const stepHeader = stepElement.find(".step-header");
                stepHeader
                    .find("strong")
                    .text(t`After:` + ` ${script.scriptName}`);
                ...
                if (result.error) {
                    stepHeader.append(
                        $(
                            `<div class='warning_text text_rose-500'>${result.error}</div>`,
                        ),
                    );
                }
        ...
        if (renderMode === "message") {
            ...
            const messageBlock = $(
                '<div class="mes"><div class="mes_text"></div></div>',
            );
    ...
    debuggerHtml.find("#regex_debugger_expand_steps").on("click", function () {
        const popupContainer = $(
            '<div class="expanded-regex-container"></div>',
        );
        const navPanel = $(
            '<div class="expanded-regex-nav"><h4>Steps</h4></div>',
        );
        ...
        $("#regex_debugger_rules .regex-debugger-rule").each(function () {
            ...
            link.on("click", function (e) {
                ...
                if (targetElement.length) {
                    const scrollTo =
                        contentPanel.scrollTop() + targetElement.position().top;
    ...
    debuggerHtml.find("#regex_debugger_expand_final").on("click", function () {
        ...
        const popupContent = $('<div class="regex-popup-content"></div>').html(
            content,
        );
...
function migrateSettings() {
    ...
    if (
        ...
        Array.isArray(extension_settings.preset_allowed_regex)
    ) {
        extension_settings.preset_allowed_regex = {};
    }
...
async function checkPresetEmbeddedRegexScripts() {
    ...
    if (Array.isArray(scripts) && scripts.length > 0) {
        if (!extension_settings.preset_allowed_regex[main_api].includes(name)) {
            ...
            if (!accountStorage.getItem(checkKey)) {
                ...
                if (result) {
                    extension_settings.preset_allowed_regex[main_api].push(
                        name,
                    );
                    saveSettingsDebounced();
                }
    ...
    const isPresetAllowed =
        extension_settings?.preset_allowed_regex[main_api]?.includes(name);
    const hasScripts = getScriptsByType(scriptTypes.PRESET).some(
...
function onPresetRenamed({ apiId, oldName, newName }) {
    ...
    const index =
        extension_settings.preset_allowed_regex[apiId].indexOf(oldName);
    if (index !== -1) {
...
jQuery(async () => {
    ...
    $("#import_regex_file").on("change", async function () {
        ...
        template
            ...
            .on("input", () => (target = scriptTypes.GLOBAL));
        template
            .find("#regex_import_target_scoped")
            .on("input", () => (target = scriptTypes.SCOPED));
        template
            .find("#regex_import_target_preset")
            .on("input", () => (target = scriptTypes.PRESET));
    ...
    $("#regex_preset_toggle").on("input", function () {
        ...
        if (isEnable) {
            ...
            const index =
                extension_settings.preset_allowed_regex[main_api].indexOf(name);
            if (index !== -1) {
                extension_settings.preset_allowed_regex[main_api].splice(
                    index,
                    1,
                );
            }
        ...
        regexScripts: () =>
            getRegexScripts().map((script) => {
                ...
                return new SlashCommandEnumValue(
                    script.scriptName,
                    `${enumIcons.getStateIcon(!script.disabled)} [${type === scriptTypes.GLOBAL ? "global" : type === scriptTypes.SCOPED ? "scoped" : "preset"}] ${script.findRegex}`,
                    type === scriptTypes.GLOBAL
                        ? enumTypes.enum
                        : enumTypes.name,
                    type === scriptTypes.GLOBAL
                        ? "G"
                        : type === scriptTypes.SCOPED
                          ? "S"
                          : "P",
                );
            }),
